        ...
        ARViewer = ARCS.Component.create(
            ...
            function () {
                ...
                this.setIntrinsics = function (parameters) {
					...
					var alphau = parameters[0];
					var suv = parameters[1];
					var u0 = parameters[2];
					var alphav = parameters[4];
					var v0 = parameters[5];
					var near = 0.01;
					var far = 10;
					var h = sourceHeight;
					var w = sourceAspectRatio * h;

					var left = (-near/alphau)*u0;
					var right = (near / alphau) * (w - u0);
					var bottom = (-near/ alphav)*v0;
					var top = (near / alphav) * (h - v0);
                ...
                this.setExtrinsics = function (markers) {
                    ...
                    for ( i = 0; i < markers.length; i++) {
                        if ( markers[i].id === sceneId ) {
							...
							matrixMarker.set(markers[i].pose.rotation[0][0], markers[i].pose.rotation[0][1], markers[i].pose.rotation[0][2],markers[i].pose.position[0], markers[i].pose.rotation[1][0], markers[i].pose.rotation[1][1],markers[i].pose.rotation[1][2],markers[i].pose.position[1], markers[i].pose.rotation[2][0], markers[i].pose.rotation[2][1], markers[i].pose.rotation[2][2], markers[i].pose.position[2],0,0,0,1);

							var invMatrixMarker = new THREE.Matrix4().getInverse(matrixMarker, true);

							invMatrixMarker.decompose(camera3d.position, camera3d.rotation, camera3d.scale);
